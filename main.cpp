...
public:
  ...
  auto generate_injection_particle(Velocity& velocity, float64 delt, float64 xbound, float64 sign)
  {
    ...
    xx                   = xbound - sign * cc * delt * uniform(mt);
    dx                   = ux * delt / lorentz_factor(ux, uy, uz, rc);

    // make sure that the particle is injected inside the simulation domain
    while ((xx + dx - xbound) * sign < 0) {
      std::tie(ux, uy, uz) = velocity(mt);
      xx                   = xbound - sign * cc * delt * uniform(mt);
      dx                   = ux * delt / lorentz_factor(ux, uy, uz, rc);
    }
  ...
  float64 initial_flow_profile(const float64 x, const float64 L)
  {
    ...
    float64 xmax = gxlim[1] - boundary_margin * delx;
    float64 ymin = +cos(0.5 * nix::math::pi * (x - xmin) / L);
    float64 ymax = -cos(0.5 * nix::math::pi * (x - xmax) / L);
    float64 fmin = (x < xmin + L) ? ymin : 0.0;
    float64 fmax = (x > xmax - L) ? ymax : 0.0;
    return fmin + fmax;
  }
  ...
  void set_boundary_particle(ParticleVec& particle) override
  {
    ...
    if (get_nb_rank(0, 0, -1) == MPI_PROC_NULL) {
      ...
      for (int is = 0; is < Ns; is++) {
        ...
        for (int ip = 0; ip < particle[is]->Np; ip++) {
          if (xu(ip, 0) <= gxlim[0]) {
            auto [x, ux, uy, uz] = generate_injection_particle(mj[is], delt, gxlim[0], +1.0);
    ...
    if (get_nb_rank(0, 0, +1) == MPI_PROC_NULL) {
      ...
      for (int is = 0; is < Ns; is++) {
        ...
        for (int ip = 0; ip < particle[is]->Np; ip++) {
          if (xu(ip, 0) >= gxlim[1]) {
            auto [x, ux, uy, uz] = generate_injection_particle(mj[is], delt, gxlim[1], -1.0);
  ...
  void inject_particle(ParticleVec& particle) override
  {
    ...
    if (get_nb_rank(0, 0, -1) == MPI_PROC_NULL) {
      ...
      // calculate number of injection particles and reallocate buffer if
      // necessary
      np_inj.fill(0);
